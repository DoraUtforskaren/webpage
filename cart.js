...
async function fetchProductsForCart() {
    try {
        ...
        //const productsArray = Array.isArray(allProducts) ? allProducts : Object.values(allProducts);
         // Filter the complete list of products based on the cartProductIds
        //const cartProducts = productsArray.filter(product => cartProductIds.includes(product.id));


        //console.log("cartProducts in fetch: ", cartProducts);
        //const cartProducts = allProducts.filter(product => cartProductIds.includes(product.id));
        return allProducts;
        /*
        // Filter products based on the product IDs in the cart
        const cartProducts = Array.isArray(allProducts)
...
const tableName2 = 'contact_info';

function createOrder() {
    const order_ids = generateOrderId();

    tempCart = cart;
    //The content for an order
    const orderItems = tempCart.map((item, index) =>({
        id: index + 1,
        products_order_id: order_ids,
        product_id: item.id,
    ...
    contact_details.orderId = order_ids;
    ...
    //sendOrderToServer(orderItems, "order_items");
    //sendOrderToServer(contact_details, "contact_info");
